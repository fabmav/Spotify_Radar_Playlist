#* main script for generating weekly streamlit update
...
#logging file setup
logging.basicConfig(filename=f'log/weekly_retrieval{TODAY}.log', level=logging.INFO) 
...
# write_to_file(liste=liste_id,file_name="stats/weekly_tracks_popularity.txt",first_line=FIRST_LINE[1],
#               string_length=49,base_url=URL_TRACKS,parse_func=get_tracks_popularity,
#               request_headers=HEADERS,request_params=PARAMS,sep=";")

# write_to_file(liste=liste_id,file_name="stats/weekly_tracks_artists.txt",first_line=FIRST_LINE[2],
...
# write_to_file(liste=liste_artists,file_name="stats/weekly_artists_data.txt",first_line=FIRST_LINE[3],
#               string_length=49,base_url=URL_ARTISTS,parse_func=get_artist_data,
#               request_headers=HEADERS)

write_to_file(liste=liste_artists,file_name="stats/stat_artists_genres_backup.txt",first_line=FIRST_LINE[4],
              string_length=49,base_url=URL_ARTISTS,parse_func=get_artist_genre,
              request_headers=HEADERS)

write_to_file(liste=liste_artists,file_name="stats/stat_artists_genres_agg_backup.txt",first_line=FIRST_LINE[5],
              string_length=49,base_url=URL_ARTISTS,parse_func=get_artist_genre_agg,
              request_headers=HEADERS,sep=";")
